...
describe('buildCapturingPattern', () => {
  ...
  describe('ambiguousRuns', () => {
    it('flags two adjacent unpadded numeric tokens with no separator', () => {
      ...
      // Runs are emitted as ONE bounded digit group (0.9.2 ReDoS
      // backport — see buildCapturingPattern): the run record carries
      // the regex group name (groupName) plus the per-token names it
      // resolves.
