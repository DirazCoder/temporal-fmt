...
test('next Tuesday said on a Tuesday returns 7 days out, not today', () => {
  ...
  assert.equal(parseRelative('next Tuesday', REFERENCE).toString(), '2026-08-11');
});

test('next Friday returns the next occurrence of Friday (3 days out from Tuesday)', () => {
  assert.equal(parseRelative('next Friday', REFERENCE).toString(), '2026-08-07');
});

test('next Monday returns the next occurrence of Monday (6 days out)', () => {
  assert.equal(parseRelative('next Monday', REFERENCE).toString(), '2026-08-10');
});
...
test('last Monday said on a Tuesday returns 7 days ago, not yesterday', () => {
  ...
  assert.equal(parseRelative('last Monday', REFERENCE).toString(), '2026-08-03');
});
...
test('last Tuesday said on a Tuesday returns 7 days ago', () => {
  ...
  assert.equal(parseRelative('last Tuesday', REFERENCE).toString(), '2026-07-28');
});
...
test('last Friday returns the most recent occurrence of Friday (5 days ago from Tuesday)', () => {
  ...
  assert.equal(parseRelative('last Friday', REFERENCE).toString(), '2026-07-31');
});
...
test('this Wednesday returns the X of the current ISO week (1 day from Tuesday)', () => {
  ...
  assert.equal(parseRelative('this Wednesday', REFERENCE).toString(), '2026-08-05');
});
...
test('this Monday returns the Monday of the current week (yesterday from Tuesday)', () => {
  ...
  assert.equal(parseRelative('this Monday', REFERENCE).toString(), '2026-08-03');
});
...
test('2 weeks ago returns -14 days', () => {
  assert.equal(parseRelative('2 weeks ago', REFERENCE).toString(), '2026-07-21');
});
...
test('5 years ago returns -5 calendar years', () => {
  assert.equal(parseRelative('5 years ago', REFERENCE).toString(), '2021-08-04');
});
...
test('month-day: "December 25" said in August resolves to this year (Christmas still upcoming)', () => {
  ...
  assert.equal(parseRelative('December 25', REFERENCE).toString(), '2026-12-25');
});
...
test('month-day with explicit year is ignored (year stripped per spec — month-day only)', () => {
  ...
  assert.equal(parseRelative('March 5th, 2030', REFERENCE).toString(), '2027-03-05');
});
...
test('this Tuesday said on a Tuesday returns today', () => {
  ...
  assert.equal(parseRelative('this Tuesday', REFERENCE).toString(), '2026-08-04');
});
...
test('parseRelative result type is PlainDate for every supported phrase', () => {
  ...
  const phrases = [
    ...
    'in 3 days', '2 weeks ago', 'in 1 month', '5 years ago',
    'March 5th', 'Dec 25',
  ];
